...
function addItem(event){
    ...
    if (value === ''){
        showAction(addItemsAction, 'Please add todo item', false);
    } else {
...
function createItem(value){
    ...
    parent.innerHTML = `<h4 class="todo-item__title">${value}</h4>
    <a href="#" class="todo-item__link">
...
function displayStorage(){
    let exists = localStorage.getItem('todoList');
    
    if(exists){
        let storageItems = JSON.parse(localStorage.getItem('todoList'));
...
function removeSingleItem(event){
    ...
    if(link.classList.contains('todo-item__link')){
